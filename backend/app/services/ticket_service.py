...
async def create_ticket(db: AsyncSession, ticket_in: TicketCreate) -> TicketResponse:
    ticket = Ticket(
        title=ticket_in.title, description=ticket_in.description, status="open"
    )
    if ticket_in.tags:
        tag_names = [
            tag_name.lower().strip() for tag_name in ticket_in.tags if tag_name.strip()
        ]
        tags = []
...
async def get_ticket(db: AsyncSession, ticket_id: int) -> Optional[TicketResponse]:
    result = await db.execute(
        select(Ticket).options(selectinload(Ticket.tags)).where(Ticket.id == ticket_id)
    )
...
async def search_tickets(
    db: AsyncSession, keyword: str, skip: int = 0, limit: int = 20
) -> List[TicketResponse]:
    result = await db.execute(
        select(Ticket)
